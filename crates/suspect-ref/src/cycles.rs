    ...
    if let Some(doc) = ws.extend_doc(d) {
            ...
            match node.kind() {
                ...
                SyntaxKind::Mapping => {
                    for (k, v) in node.mapping_entries() {
                        if let Some(v) = v {
                            let tok = String::from_utf8_lossy(k.scalar_bytes())
                                .to_string()
                                .into_boxed_str();
        ...
        for (i, edge) in edges.iter().enumerate() {
            let tr = match &edge.parsed {
                ParsedRef::Local(p) if !p.is_root() => match resolved.get(p.to_path().as_str()) {
                    Some(r) => r.clone(),
                    None => continue,
                },
                _ => continue,
...
fn classify(pairs: &[(Step, usize)], edges: &[RefEdge]) -> CycleKind {
    for (_, idx) in pairs {
        ...
        let legal = edge
            ...
            .tokens()
            .iter()
            .any(|t| RECURSION_POINTS.contains(&&**t));
        if !legal {
