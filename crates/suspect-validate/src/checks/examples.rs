...
/// TypeSet bits a value can legitimately inhabit; mathematical integers
/// satisfy both `integer` and `number`, including decimal/exponent spellings.
fn compatible_bits(value: NodeRef<'_>) -> u8 {
    ...
    match value.kind() {
        ...
        ValueKind::Float if value.is_integral_number() => TypeSet::INTEGER | TypeSet::NUMBER,
...
/// against), or the type set is empty. Explicit null examples are checked;
/// missing YAML values have no example node and are handled as absent above.
fn check_media_example(api: &OpenApi<'_>, media: &MediaType<'_>, out: &mut Vec<Diagnostic>) {
    ...
    let Some(set) = schema.type_set_for(api.version()) else {
        return;
    };
    if set.is_empty() {
        return;
    ...
    if compatible_bits(example) & set.bits() == 0 {
        out.push(diag_at(
            example,
