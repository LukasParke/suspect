...
pub(super) fn payload_type(payload: &Payload) -> String {
    match payload {
        ...
        Payload::Stream {
            ...
            Some(model) => format!("std::vec::Vec<crate::models::{model}>"),
            // A schemaless stream surfaces untyped parsed envelope values.
            None => "std::vec::Vec<crate::JsonValue>".into(),
        },
        Payload::Stream {
            model,
            request: false,
            ...
            Some(model) => format!("crate::http::ItemStream<crate::models::{model}>"),
            // A schemaless stream surfaces untyped parsed envelope values.
            None => "crate::http::ItemStream<crate::JsonValue>".into(),
        },
...
fn payload_descriptor(payload: &Payload) -> Value {
    match payload {
        ...
        Payload::Stream { model, request, .. } => {
            json!({"kind":if *request {"finite-items"}else{"item-stream"},"model":model.as_deref()})
        }
        Payload::Parts(a) => {
...
pub(super) fn package(plan: &HttpPlan, package: &PackageConfig, crate_name: &str) -> Vec<OutFile> {
    ...
    // Pagination support is generated only when the plan carries paginated
    // operations; unconfigured plans gain no artifact at all.
    let pagination = pagination::emission(plan);
    // Generated OAuth support follows the same rule: only plans carrying
    // executable compiled schemes gain the module and its client methods.
    let oauth = oauth::emission(plan);
    // Generated typed event streams follow the same rule: only plans carrying
    // a discriminated SSE stream operation gain the module and its client
    // methods.
    let stream_events = stream_events::emission(plan);
    // Generated incoming receipt helpers follow the same rule: only plans
    // carrying declared webhooks/callbacks gain the module, and plans without
    // them gain no artifact at all.
    let incoming = incoming::emission(plan);
    files.insert(
    ...
    files.insert(
        "rust/src/attribution.rs".into(),
        match &plan.config.attribution {
            Some(attribution) => format!(
                "//! Code generated by suspect. DO NOT EDIT.\n//! ua/v1 attribution: every request identifies suspect as the generator and the\n//! SDK or a caller-supplied application as the client.\n\npub(crate) const ATTRIBUTION_SUSPECT_VERSION: &str = {:?};\npub(crate) const ATTRIBUTION_SDK_NAME: &str = {:?};\npub(crate) const ATTRIBUTION_SDK_VERSION: &str = {:?};\npub(crate) const ATTRIBUTION_SPEC_VERSION: &str = {:?};\n",
                attribution.suspect_version,
                attribution.sdk_name,
                attribution.sdk_version,
                attribution.spec_version,
            ),
    ...
    if let Some(pagination) = &pagination {
        files.insert("rust/src/pagination.rs".into(), pagination.module.clone());
    }
    // Generated OAuth support gains exactly its module; everything else stays
    // byte-identical to the unconfigured emission.
    if let Some(oauth) = oauth {
        files.insert("rust/src/oauth.rs".into(), oauth.module);
    }
    // Generated typed event streams gain exactly their module; everything else
    // stays byte-identical to the unconfigured emission.
    if let Some(stream_events) = &stream_events {
        files.insert(
            "rust/src/stream_events.rs".into(),
            stream_events.module.clone(),
        );
    }
    // Generated incoming receipt helpers gain exactly their module; everything
    // else stays byte-identical to the unconfigured emission.
    if let Some(incoming) = &incoming {
        files.insert("rust/src/incoming.rs".into(), incoming.module.clone());
    }
    files.insert(
...
fn root(
    plan: &HttpPlan,
    crate_name: &str,
    pagination: Option<&pagination::Emission>,
    oauth: Option<&oauth::Emission>,
    stream_events: Option<&stream_events::Emission>,
    incoming: Option<&incoming::Emission>,
) -> String {
