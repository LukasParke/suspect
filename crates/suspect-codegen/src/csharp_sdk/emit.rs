...
pub(super) fn package(plan: &SdkPlan) -> Result<Vec<OutFile>, Vec<HttpDiagnostic>> {
    ...
    let mut files = vec![
        ...
        file("src/Attribution.g.cs", attribution_source(plan)),
        file(
    ...
    if super::pagination::emits(plan) {
        files.push(file("src/Pagination.g.cs", super::pagination::module(plan)));
    }
    if super::oauth::emits(plan) {
        files.push(file("src/OAuth.g.cs", super::oauth::module(plan)));
    }
    if super::stream_events::emits(plan) {
        files.push(file(
            "src/StreamEvents.g.cs",
            super::stream_events::module(plan),
        ));
    }
    if super::incoming::emits(plan) {
        files.push(file("src/Incoming.g.cs", super::incoming::module(plan)));
    }
    if resources {
...
/// `ua/v1` attribution constants compiled at generation time; the language
/// version is discovered at client construction by `HttpRuntime`. An absent
/// descriptor emits the disabled sentinel: an empty suspect version.
fn attribution_source(plan: &SdkPlan) -> String {
    let values = match plan.attribution() {
        Some(attribution) => format!(
            "internal static class Attribution\n{{\n    // ua/v1 attribution: every request identifies suspect as the generator and the\n    // SDK or a caller-supplied application as the client.\n    internal const string SuspectVersion = {};\n    internal const string SdkName = {};\n    internal const string SdkVersion = {};\n    internal const string SpecVersion = {};\n    internal const string Language = {};\n}}\n",
            quote(&attribution.suspect_version),
            quote(&attribution.sdk_name),
            quote(&attribution.sdk_version),
            quote(&attribution.spec_version),
            quote(&attribution.language),
        ),
        None => "internal static class Attribution\n{\n    // An empty suspect version disables the automatic attribution header.\n    internal const string SuspectVersion = \"\";\n    internal const string SdkName = \"\";\n    internal const string SdkVersion = \"\";\n    internal const string SpecVersion = \"\";\n    internal const string Language = \"\";\n}\n".into(),
    };
    format!("{}{}", header(plan), values)
}
